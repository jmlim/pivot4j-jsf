...
public class DrillDownOnPositionHandler extends
		AbstractDrillDownHandler<DrillExpandPosition> {

	/**
	 * @see com.eyeq.pivot4j.ui.primefaces.command.DrillDownHandler#getTransformType()
	 */
	@Override
	public Class<DrillExpandPosition> getTransformType() {
	...
	public DrillDownParameters createDrillDownParameters(BuildContext context) {
		...
		DrillExpandPosition transform = createTransform(context);

		boolean drillDown;
		if (transform.canExpand(position, member)) {
			drillDown = true;
		} else if (transform.canCollapse(position, member)) {
			drillDown = false;
		} else {
			return null;
		}
	...
	public CommandButton createCommand(DrillDownParameters parameters,
		...
		if (parameters.isDrillDown()) {
			button.setIcon("ui-icon-triangle-1-e");
		} else {
			button.setIcon("ui-icon-triangle-1-s");
		}
	...
	public void executeCommand(PivotModel model, FacesContext context) {
		...
		DrillExpandPosition transform = model.getTransform(getTransformType());
		if (drillDown) {
